...
class OidcAppLifecycleOperator(base.AppLifecycleOperator):
    def app_lifecycle_actions(self, context, conductor_obj, app_op, app,
        ...
        if (
            ...
            self.post_apply_operation(context, conductor_obj, app)
            # During upgrades, operation/post does not fire,
            # so also attempt post_apply here. If conductor_obj is None
            # (always is for manifest/post), this will create a
    ...
    def pre_apply_operation(self, context, conductor_obj, app):
        ...
        Behavior:
        - Strips any stale TLS parameters from user_overrides (TLS is
          managed exclusively by system_overrides via the helm plugin).
        ...
        # TLS params belong in system_overrides only (regenerated by
        # the helm plugin on every apply). Remove any stale TLS values
        # from user_overrides left by older releases.
        self._strip_tls_from_user_overrides(dbapi_instance)
    ...
    def post_apply(self, context, conductor_obj):
        ...
        fresh install or upgrades where direct trigger is unsafe, a flag
        file is created so the conductor periodic audit triggers the puppet
        manifest once the system is stable.

        :param context: Request context provided by the conductor.
        :param conductor_obj: Sysinv conductor manager instance.
        """
        # During fresh install / initial-unlock, do NOT trigger the puppet
        # manifest directly as the system is not stable yet and a deferred
        # manifest would restart keystone at a time when ansible playbooks
        # depend on it being available. Instead, create a flag file so the
        # conductor periodic audit triggers it once the system stabilizes.
        if not os.path.isfile(tsc.INITIAL_CONFIG_COMPLETE_FLAG):
            LOG.info("Initial config not complete, deferring Keystone "
                     "federation trigger for conductor audit")
            try:
        ...
        if conductor_obj is not None:
            ...
            # operation/post hook).
            # Co-apply keystone federation AND horizon runtime so that the
            # WebSSO settings file (_31_websso_settings.py) is generated by
            # horizon once oidc-issuer-url is configured. horizon::runtime is
            # gated on the oidc-issuer-url hiera key and skipped on subclouds,
            # so it is a no-op where WebSSO does not apply. Both classes share
            # a single config target update (no extra config-out-of-date
            # churn), and this whole branch is protected by the federation
            # marker guard above so it only runs when federation is (re)set up.
            config_dict = {
            ...
            LOG.info("Triggered Keystone federation and Horizon WebSSO "
                     "configuration after oidc-auth-apps apply")
        else:
    ...
    def _configure_oidc_client_override(self, dbapi_instance):
        ...
        TLS version and cipher suites are NOT included here — they are
        managed exclusively by the helm plugin (oidc_client.py) via
        system_overrides, which are regenerated on every apply. Placing
        TLS params in user_overrides would override system_overrides and
        prevent runtime TLS changes from taking effect.
    ...
    def _configure_dex_override(self, dbapi_instance, mgmt_ip, ldap_pwd):
        ...
        TLS minimum version is NOT included here — it is managed
        exclusively by the helm plugin (dex.py) via system_overrides.
    ...
    def _strip_tls_from_user_overrides(self, dbapi_instance):
        """Remove TLS parameters from oidc-client and dex user_overrides.

        TLS enforcement (tlsMinVersion, tlsCipherSuites) is managed
        exclusively by system_overrides generated by the helm plugins
        (oidc_client.py and dex.py) on every apply. Older releases
        incorrectly wrote these values into user_overrides, which take
        higher priority and prevent runtime TLS changes from taking
        effect. This method cleans up any stale TLS entries.
        ...
        # Strip from oidc-client
        self._strip_tls_keys_from_chart(
            dbapi_instance, db_app.id,
            app_constants.HELM_CHART_OIDC_CLIENT,
            config_keys=['tlsMinVersion', 'tlsCipherSuites'],
        )

        # Strip from dex (web.tlsMinVersion)
        self._strip_tls_keys_from_dex(dbapi_instance, db_app.id)

    def _strip_tls_keys_from_chart(self, dbapi_instance, db_app_id,
                                   chart_name, config_keys):
        """Remove specific keys from the 'config' section of a chart's
        user_overrides.

        :param sysinv.db.api.DbApi dbapi_instance: Sysinv database API.
        :param int db_app_id: Application database ID.
        :param str chart_name: Helm chart name.
        :param list config_keys: Keys to remove from overrides['config'].
        """
        try:
            helm_override = dbapi_instance.helm_override_get(
                app_id=db_app_id,
            ...
            if changed:
                ...
                dbapi_instance.helm_override_update(
                    db_app_id, chart_name, common.HELM_NS_KUBE_SYSTEM,
                    {'user_overrides': user_overrides},
                )
                LOG.info("Stripped stale TLS params from %s user_overrides",
                         chart_name)

        except exception.NotFound:
            pass
        except Exception as e:
            LOG.warning("Failed to strip TLS from %s user_overrides: %s",
                        chart_name, e)

    def _strip_tls_keys_from_dex(self, dbapi_instance, db_app_id):
        """Remove tlsMinVersion from dex user_overrides config.web section."""
        try:
            helm_override = dbapi_instance.helm_override_get(
                app_id=db_app_id,
            ...
            overrides = yaml.safe_load(helm_override.user_overrides)
            web = overrides.get('config', {}).get('web', {})
            changed = False

            if 'tlsMinVersion' in web:
                del web['tlsMinVersion']
                changed = True
            ...
            if changed:
                user_overrides = yaml.safe_dump(
                    overrides, default_flow_style=False, sort_keys=False)
                dbapi_instance.helm_override_update(
                    db_app_id, app_constants.HELM_CHART_DEX,
                    common.HELM_NS_KUBE_SYSTEM,
                    {'user_overrides': user_overrides},
                )
                LOG.info("Stripped stale TLS params from dex user_overrides")
